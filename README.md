...
> `cli.py`/`evaluator.py`/`tools.py`/`extra_tools.py`/`backends.py`/…)—— 有自己的工具循环和
...
`invoke_skill` 调用。参考 `skills/*.md` 里现有的 9 个写法。
...
所有实现都**扁平铺在仓库根目录**(没有嵌套的 Python 包):
...
│   └── *.md                           # 9 个引擎 workflow(invoke_skill 调用)
│
