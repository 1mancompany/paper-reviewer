...
>
> **Engine-backed talent**:不同于纯技能 talent,它自带真正的 agentic 引擎(扁平铺在仓库根目录的
> `cli.py`/`evaluator.py`/`review_tools.py`/`extra_tools.py`/`backends.py`/…)—— 有自己的工具循环和
> LLM 后端(Anthropic / OpenAI-compatible)。
...
外层是 talent-template 规范的 wrapper;agentic 引擎的实现**扁平铺在仓库根目录**(没有嵌套 Python 包):

```
paper-reviewer/
│   # ── talent-template wrapper ──
├── profile.yaml             # Talent 身份(skills 含 'peer_reviewer' 流水线匹配键)
├── DESCRIPTION.md           # Talent Market 展示文案(Overview/Use Cases/…)
├── avatar.jpg
...
├── tools/
│   ├── manifest.yaml                  # 工具声明
│   └── research-eval/                 # 引擎作为 custom tool
│       ├── TOOL.md / manifest.yaml / run.sh
├── manifest.json            # (可选)前端设置 UI(API keys / 审计行为)
├── prompts/talent_persona.md# (附加)人格 + 行为契约
├── vessel/vessel.yaml       # (附加)超时 3600s / 迭代 200 / sandbox
│
│   # ── agentic 引擎(真正的实现,扁平在根目录)──
├── cli.py                   # CLI 入口(`research-eval` console script / `python cli.py`)
